...
public class NOC extends InterConnect {
	...
	public NOC(NocConfig nocConfig) {
		...
		if (nocConfig.chipletConnection == CHIPLETCONNECTIONTYPE.PARALLEL && nocConfig.ischiplet == true){
			...
			//Now creating the Noc interface for the case of chiplets, where 
			//an additional parameter of chiplet number will also be passed along
	...
	private NocInterface createNocInterface(int i, int j,int chipletId) {
		NocInterface nocInterface = new NocInterface(nocConfig,chipletId);
		ID id = new ID(i, j);
		nocInterface.setId(id);
		return nocInterface;
	}
	...
	public void resetNumAccesses()
	{
		for(int i = 0; i < this.numRows; i++) {
			for(int j = 0; j < this.numColumns; j++) {
				networkElements[i][j].getRouter().resetNumAccesses();
			}
		}
	}
